...
public class ATVision {
    ...
    PhotonCamera atcamera = new PhotonCamera("AprilTag Camera"); // Update UI with this info
    ...
    Transform3d cameraToRobot = new Transform3d(new Translation3d(Units.feetToMeters(1), 0.0, Units.feetToMeters(1)), new Rotation3d(0,0,0)); //Cam mounted facing forward, half a meter forward of center, half a meter up from center.
    ArrayList<Pair<PhotonCamera, Transform3d>> camList = new ArrayList<Pair<PhotonCamera, Transform3d>>();
    Optional<Pose3d> optionalFieldRelativeTagPose;
    Pose3d fieldRelativeTagPose;

    RobotPoseEstimator robotPoseEstimator = new RobotPoseEstimator(aprilTagFieldLayout, PoseStrategy.LOWEST_AMBIGUITY, camList);
    ...
    //returns your robot’s Pose3d on the field using the pose of the AprilTag relative to the camera, pose of the AprilTag relative to the field, and the transform from the camera to the origin of the robot.
    public Pose3d getRobotPose(){
        ...
        if(result.hasTargets()){
            ...
            if (optionalFieldRelativeTagPose.isPresent()) {
                ...
                return PhotonUtils.estimateFieldToRobotAprilTag(cameraToTarget, fieldRelativeTagPose, cameraToRobot);
